...
class ImageInfo:
    ...
    local_path: Path | None  # Resolved local file path (None if URL or not found)
...
class ParseResult:
    ...
    source_file: Path | None = None  # Original markdown file path
...
class MarkdownParser:
    ...
    def parse(self, content: str, source_file: Path | None = None) -> ParseResult:
        """
    ...
    def parse_file(self, file_path: Path) -> ParseResult:
        ...
        if not file_path.exists():
            ...
            return ParseResult(
                html="", warnings=[f"File not found: {file_path}"], source_file=file_path
            )
        ...
        try:
            ...
            return ParseResult(
                html="", warnings=[f"Failed to read file: {e}"], source_file=file_path
            )

        return self.parse(content, source_file=file_path)

    def _extract_images(self, html: str, source_file: Path | None) -> list[ImageInfo]:
        """
    ...
    def _analyze_image_src(self, src: str, alt: str, source_file: Path | None) -> ImageInfo:
        """
    ...
    def update_image_paths(self, html: str, path_mapping: dict) -> str:
        """
...
def parse_markdown(content: str, source_file: Path | None = None) -> ParseResult:
    """
